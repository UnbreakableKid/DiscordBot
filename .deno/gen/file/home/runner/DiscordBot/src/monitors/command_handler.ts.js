...
import { bgBlack, bgGreen, bgMagenta, bgYellow, black, botId, cache, deleteMessages, green, red, white } from "../../deps.ts";
...
export function logCommand(message, guildName, type, commandName) {
    const command = `[COMMAND: ${bgYellow(black(commandName || "Unknown"))} - ${bgBlack([
        ...
        "Slowmode",
        "Missing"
    ].includes(type) ? red(type) : type === "Success" ? green(type) : white(type))}]`;
...
async function parseArguments(message, // deno-lint-ignore no-explicit-any
command, parameters) {
    const args = {
    };
    ...
    for (const argument of command.arguments){
        ...
        if (Object.prototype.hasOwnProperty.call(argument, "defaultValue")) {
            ...
            const question = await message.reply(translate(message.guildId, "strings:MISSING_REQUIRED_ARG", {
                name: argument.name,
                type: argument.type === "subcommand" ? command.subcommands?.map((sub)=>sub.name
                ).join(", ") || "subcommand" : argument.type
            })).catch(log.error);
            if (question) {
                const response = await needMessage(message.authorId, message.channelId).catch(log.error);
                if (response) {
                    const responseArg = await resolver.execute(argument, [
                        response.content
                    ], message, command);
                    if (responseArg) {
...
/** Runs the inhibitors to see if a command is allowed to run. */ async function commandAllowed(message, // deno-lint-ignore no-explicit-any
command) {
...
async function executeCommand(message, // deno-lint-ignore no-explicit-any
command, parameters) {
    try {
...
bot.monitors.set("commandHandler", {
    name: "commandHandler",
    ignoreDM: false,
    /** The main code that will be run when this monitor is triggered. */ // deno-lint-ignore require-await
