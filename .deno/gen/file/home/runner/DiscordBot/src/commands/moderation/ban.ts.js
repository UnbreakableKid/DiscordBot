  ...
  execute: async (message, args) => {
    try {
      ...
      if (!(canBotBanMember && canAuthorBanMember)) {
        const embed = new Embed().setColor("#F04747").setTitle("Could not Ban")
          .setDescription(
      ...
      try {
        const embed = new Embed().setColor("#F04747").setTitle(
          `Banned from ${message.member?.guild.name}`,
        ).addField("Banned By:", `<@${authorId}>`).addField(
          "Reason:",
          args.reason,
        ).setTimestamp();
      ...
      if (!banned) return;
      const embed = new Embed().setColor("#F04747").setTitle(`Banned User`)
        .setThumbnail(args.member.avatarURL).addField(
          "User:",
          args.member.mention,
          true,
        ).addField("Banned By:", `<@${authorId}>`, true).addField(
          "Reason:",
          args.reason,
