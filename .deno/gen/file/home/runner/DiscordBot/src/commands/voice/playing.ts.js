...
createCommand({
    ...
    execute (message) {
        ...
        const embed = new Embed().setAuthor(`${message.tag}`, message.member?.avatarURL).setTitle(player.playing && queue ? `Now Playing - ${queue[0].info.title}` : `Not playing anything`, player.playing && queue ? queue[0].info.uri : "").setDescription(player.playing && queue ? `**Progress:** ${getMusicLength(player.position)}/${getMusicLength(queue[0].info.length)}` : `You're not playing any music, add a music using im!play (music)`).setTimestamp(player.timestamp);
        return message.send({
            embed
        });
    }
