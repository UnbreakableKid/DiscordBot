...
/** Convert milliseconds to MM:SS */ export function getMusicLength(milliseconds) {
    return milliseconds > 3600000 ? new Date(milliseconds).toISOString().substr(11, 8) : new Date(milliseconds).toISOString().substr(14, 5);
}
...
export async function addSoundToQueue(message, track) {
    ...
    if (bot.musicQueues.has(message.guildId)) {
        ...
        await message.reply(`Added ${track.info.title} to the queue! Position in queue: ${bot.musicQueues.get(message.guildId).length - 1}`);
    } else {
...
export async function checkIfUserInMusicChannel(message, player) {
    ...
    if (player.channel !== cache.guilds.get(message.guildId)?.voiceStates.get(message.authorId)?.channelId?.toString()) {
        await message.reply(`You need to been in the same voice channel than the bot!`);
        return false;
    }
    return true;
}
