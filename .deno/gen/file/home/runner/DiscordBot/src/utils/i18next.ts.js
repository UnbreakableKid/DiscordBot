...
/** This function helps translate the string to the specific guilds needs. */ export function translate(
  guildId,
  key,
  options,
) {
  ...
  return i18next.use(Backend).init({
    ...
    missingKeyHandler: async function (lng, ns, key, fallbackValue) {
      ...
      await sendWebhook(
        ...
        configs.webhooks.missingTranslation.token, // deno-lint-ignore ban-ts-comment
        // @ts-ignore
        {
          content: response,
        },
    ...
    preload: languageFolder.map((file) =>
      file.isDirectory ? file.name : undefined
