...
/** To edit your response to a slash command. If a messageId is not provided it will default to editing the original response. */ export async function editSlashResponse(token, options) {
    ...
    const result = await rest.runMethod("patch", options.messageId ? endpoints.WEBHOOK_MESSAGE(applicationId, token, options.messageId) : endpoints.INTERACTION_ORIGINAL_ID_TOKEN(applicationId, token), options);
