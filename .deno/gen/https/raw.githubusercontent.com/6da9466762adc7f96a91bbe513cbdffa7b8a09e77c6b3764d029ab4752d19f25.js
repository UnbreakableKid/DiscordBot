...
export class Socket {
   ...
   */ get penalties() {
    ...
    if (this.stats.frameStats?.deficit != -1) {
      deficit =
        Math.pow(1.03, 500 * ((this.stats.frameStats?.deficit ?? 0) / 3000)) *
          600 - 600;
      nulled =
        (Math.pow(1.03, 500 * ((this.stats.frameStats?.nulled ?? 0) / 3000)) *
            600 - 600) * 2;
      nulled *= 2;
    }
   ...
   * @private
   */ async _open() {
    await this._processQueue().then(() => this.configureResuming()).catch((e) =>
      this.manager.emit("socketError", this, e)
    );
