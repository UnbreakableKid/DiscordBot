...
/** Update a channel's settings. Requires the `MANAGE_CHANNELS` permission for the guild. */ export async function editChannel(
  channelId,
  options,
  reason,
) {
  ...
  if (channel) {
    ...
    if (
      hasOwnProperty(options, "permissionOverwrites") &&
  ...
  const payload = {
    ...
    permission_overwrites: hasOwnProperty(options, "permissionOverwrites")
...
function processEditChannelQueue() {
  ...
  editChannelNameTopicQueue.forEach(async (request) => {
    ...
    if (!details) return;
    await editChannel(details.channelId, details.options).then((result) =>
      details.resolve(result)
    ).catch(details.reject);
    const secondDetails = request.items.shift();
    if (!secondDetails) return;
    await editChannel(secondDetails.channelId, secondDetails.options).then((
      result,
    ) => secondDetails.resolve(result)).catch(secondDetails.reject);
    return;
  });
