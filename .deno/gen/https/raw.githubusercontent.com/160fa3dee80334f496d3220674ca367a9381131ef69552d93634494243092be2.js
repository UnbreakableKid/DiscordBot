...
/** Allows easy way to add a prop to a base object when needing to use complicated getters solution. */ // deno-lint-ignore no-explicit-any
...
/** @private */ function validateSlashOptionChoices(choices, optionType) {
    for (const choice of choices){
        ...
        if (!validateLength(choice.name, {
            min: 1,
            max: 100
        })) {
            throw new Error(Errors.INVALID_SLASH_OPTIONS_CHOICES);
        }
        if (optionType === DiscordApplicationCommandOptionTypes.String && (typeof choice.value !== "string" || choice.value.length < 1 || choice.value.length > 100) || optionType === DiscordApplicationCommandOptionTypes.Integer && typeof choice.value !== "number") {
            throw new Error(Errors.INVALID_SLASH_OPTIONS_CHOICES);
        }
    }
}
/** @private */ function validateSlashOptions(options) {
    for (const option of options){
        ...
        if (option.choices?.length && (option.choices.length > 25 || option.type !== DiscordApplicationCommandOptionTypes.String && option.type !== DiscordApplicationCommandOptionTypes.Integer)) {
            throw new Error(Errors.INVALID_SLASH_OPTIONS_CHOICES);
        }
        if (!validateLength(option.name, {
            min: 1,
            max: 32
...
export function validateSlashCommands(commands, create = false) {
    for (const command of commands){
        ...
        if (command.name && (!SLASH_COMMANDS_NAME_REGEX.test(command.name) || command.name.toLowerCase() !== command.name) || create && !command.name) {
            throw new Error(Errors.INVALID_SLASH_NAME);
        }
        if (command.description && !validateLength(command.description, {
            min: 1,
            max: 100
...
/** TS save way to check if a property exists in an object */ // deno-lint-ignore ban-types
...
export function validateComponents(components) {
    ...
    for (const component of components){
        ...
        if (isButton(component)) {
            ...
            if (!validateLength(component.label, {
                max: 80
            })) {
                throw new Error(Errors.COMPONENT_LABEL_TOO_BIG);
            }
            if (component.customId && !validateLength(component.customId, {
                max: 100
            })) {
