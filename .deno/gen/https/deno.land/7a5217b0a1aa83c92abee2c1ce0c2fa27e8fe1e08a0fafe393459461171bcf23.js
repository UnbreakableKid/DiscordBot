...
/* compression levels */ //const Z_NO_COMPRESSION      = 0;
 ...
 */ function longest_match(s, cur_match) {
    ...
    let limit = s.strstart > s.w_size - MIN_LOOKAHEAD ? s.strstart - (s.w_size - MIN_LOOKAHEAD) : 0/*NIL*/ ;
 ...
 */ function deflate_stored(s, flush) {
   ...
   */ let max_block_size = 65535;
    ...
    /* Copy as much as possible from input to output: */ for(;;){
        ...
        s.lookahead = 0;
        /* Emit a stored block if pending_buf will be full: */ let max_start = s.block_start + max_block_size;
 ...
 */ function deflate_slow(s, flush) {
     ...
     */ s.prev_length = s.match_length;
       ...
       * of the string with itself at the start of the input file).
       */ s.match_length = longest_match(s, hash_head);
            /* longest_match() sets match_start */ if (s.match_length <= 5 && (s.strategy === Z_FILTERED || s.match_length === MIN_MATCH && s.strstart - s.match_start > 4096)) {
...
export class DeflateState {
   ...
   */ level = 0;
    strategy = 0;
    good_match = 0;
    /* Use a faster search when the previous match is longer than this */ nice_match = 0;
...
export function deflate(strm, flush) {
    ...
    s = strm.state;
    if (!strm.output || !strm.input && strm.avail_in !== 0 || s.status === FINISH_STATE && flush !== STATUS.Z_FINISH) {
   ...
   */ if (strm.avail_in !== 0 || s.lookahead !== 0 || flush !== STATUS.Z_NO_FLUSH && s.status !== FINISH_STATE) {
