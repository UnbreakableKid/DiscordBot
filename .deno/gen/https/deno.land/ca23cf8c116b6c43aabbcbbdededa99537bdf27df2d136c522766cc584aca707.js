...
export function inflate(strm, flush) {
  ...
  if (
    !strm || !strm.state || !strm.output || !strm.input && strm.avail_in !== 0
  ) {
    return Z_STREAM_ERROR;
  }
  state = strm.state;
    ...
    switch (state.mode) {
        ...
        if (state.flags & 1024) {
          ...
          if (copy) {
            if (state.head) {
              ...
              /*len + copy > state.head.extra_max - len ? state.head.extra_max : copy,*/ state
                .head.extra.set(input.subarray(next, next + copy), len);
      ...
      case CODELENS:
        ...
        /* build code tables -- note: do not change the lenbits or distbits
           ...
           concerning the ENOUGH constants, which depend on those values */ state
          .lenbits = 9;
        opts = {
          bits: state.lenbits,
        };
  ...
  strm.data_type = state.bits + (state.last ? 64 : 0) + (state.mode === TYPE
    ? 128
    : 0) +
...
export function inflateSetDictionary(strm, dictionary) {
  ...
  /* check for correct dictionary identifier */ if (state.mode === DICT) {
    ...
    /* dictid = adler32(dictid, dictionary, dictLength); */ dictid = adler32(
      dictid,
      dictionary,
      dictLength,
      0,
