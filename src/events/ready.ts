...
bot.eventHandlers.ready = async function () {
  ...
  function sendMufasa() {
    ...
    sendWebhook(
      snowflakeToBigint(configs.webhooks.mufasa.id!),
      configs.webhooks.mufasa.token!,
      {
        content: `It's friday! ${
  ...
  async function sendAOTD() {
    ...
    sendWebhook(
      snowflakeToBigint(configs.webhooks.AOTD.id!),
      configs.webhooks.AOTD.token!,
      {
        content: `@here Today's album of the day is ${stuff.name} by ${
          stuff.artists[0].name
        }! ${album}`,
      },
    );
